...
export function CreationEditor({ api, projectId, id, onBack, onSaved, onOpen, profileRevision }: {
  ...
  return <section className="creation-editor" aria-label="脚本编辑器">
    ...
    {final && <div className="creation-final-state"><span>已定稿 v{final.number}{finalChanged ? ' · 当前正在编辑新草稿，定稿保持不变' : ' · 与当前正文一致'}</span><button type="button" className="creation-text-action" disabled={busy} onClick={() => setExporting(final)}><Download size={13} />查看并导出定稿</button></div>}
    ...
    <div className="creation-editor-columns"><main className="creation-manuscript"><label className="creation-title-label">内容标题<input aria-label="内容标题" maxLength={255} value={draft.title} disabled={busy} onChange={event => state.update({ title: event.target.value })} /></label>
      ...
      <CreationReferencePicker api={api} projectId={projectId} value={draft.referenceSelection} onChange={referenceSelection => state.update({ referenceSelection })} disabled={busy || state.conflict} compact />
    ...
    </main><CreationAssistant locked={busy || state.conflict} api={api} projectId={projectId} draft={draft} persistedDraft={detail.item} messages={detail.messages} profileRevision={profileRevision} selection={selection} flush={state.flush} onAdopt={(body, suggestion) => { const sources = suggestion.replacement ? Array.from(new Map([...draft.sources, ...suggestion.sources].map(source => [source.id, source])).values()) : suggestion.sources; if (sources.length > 100) { setNotice('这次局部修改会超过100条参考来源，请改用整稿修改来整理引用；正文已保留。'); return false; } state.update({ body, kind: 'script', sources }); setSelection(undefined); return true; }} /></div>
  </section>;
}
