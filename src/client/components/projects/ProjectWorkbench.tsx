...
import { creationReferenceSelectionSchema, type CreationReferenceSelection, type ProjectCreation } from '../../../shared/api/project-creations.js';
...
export function ProjectWorkbench({ api, projectId, files, onEditingChange }: { api: ReadConsoleApi; projectId: string; files: ReactNode; onEditingChange?(editing: boolean): void }) {
  ...
  try { const parsed = creationReferenceSelectionSchema.safeParse(JSON.parse(referenceJson)); if (parsed.success) references = parsed.data; } catch { /* Empty/invalid recovery keeps automatic discovery. */ }
  ...
  async function planTopics() {
    ...
    const submittedReferences = structuredClone(references);
    try {
      ...
      if (result.value.profileRevision !== undefined && profileRevisionRef.current !== undefined && result.value.profileRevision !== profileRevisionRef.current) {
        setError('项目创作档案已变化，请按最新档案重新策划。'); return;
      }
      let count = 0;
      for (const topic of result.value.topics) {
        if (request.signal.aborted || !alive.current) break;
        if (result.value.profileRevision !== undefined && profileRevisionRef.current !== undefined && result.value.profileRevision !== profileRevisionRef.current) {
          setError(`已保存 ${count} 条选题。项目创作档案已变化，剩余选题未保存，请按最新档案重新策划。`); return;
        }
        const saved = await creations.create(projectId, { kind: 'topic', ...topic, brief: submittedBrief, sources: result.value.sources, referenceSelection: submittedReferences });
        if (!alive.current) return;
  ...
  return <section className="project-workbench" aria-label="项目创作工作台">
    ...
    {selected ? <CreationEditor key={`${projectId}:${selected}`} api={api} projectId={projectId} id={selected} profileRevision={profileRevision} onBack={() => { setSelected(undefined); void load(); }} onSaved={update} onOpen={setSelected} /> : <><nav className="creation-tabs" role="tablist" aria-label="项目工作台">{tabs.map(tab => <button key={tab.id} type="button" role="tab" aria-selected={view === tab.id} onClick={() => setView(tab.id)}>{tab.label}{tab.id === 'topics' && items.filter(item => item.kind === 'topic').length > 0 && <span>{items.filter(item => item.kind === 'topic').length}</span>}</button>)}</nav>
    ...
    {planning && <section className="creation-planner" aria-label="策划短视频选题"><label className="creation-label">这次想做什么内容？<textarea aria-label="选题要求" value={brief} maxLength={4000} onChange={event => setBrief(event.target.value)} placeholder="例如：面向准备美术集训的家长，从项目资料里找5个适合60秒口播的选题" rows={3} /></label><CreationReferencePicker api={api} projectId={projectId} value={references} onChange={value => setReferenceJson(JSON.stringify(value))} disabled={busy} /><div className="creation-actions"><button type="button" className="projects-button projects-button--primary" disabled={busy || !brief.trim()} onClick={() => void planTopics()}>根据资料策划</button>{busy && <button type="button" className="projects-button" onClick={() => { controller.current?.abort(); setBusy(false); }}>停止策划</button>}</div><small>沿用已保存的项目创作档案。点击后，相关资料会发送给已配置的 DeepSeek，选题保存在本机。</small></section>}
