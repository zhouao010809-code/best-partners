...
import { ExtractionHistory } from '../components/ExtractionHistory.js';
...
export function ReadOnlyQueuePage() {
  ...
  return (
    <section className="instrument-panel workspace-panel live-list-page" aria-labelledby="queue-results-title">
      ...
      <header className="results-heading">
        <div><p>EXTRACTION QUEUE</p><h2 id="queue-results-title" ref={resultsHeadingRef} tabIndex={-1}>材料结果</h2></div>
        ...
        <ul className="record-list" aria-label="材料结果">
          {data.items.map((item) => (
            <li key={item.path}>
              ...
              {runtime.api.extraction && item.processingStatus === '已归档' && item.knowledgeStatus === '未提炼' && <Link className="quiet-button queue-extraction-action" aria-label={`提炼 ${item.title}`} to={extractionHref(item.path)}>提炼 / 查看候选</Link>}
            </li>
          ))}
        </ul>
