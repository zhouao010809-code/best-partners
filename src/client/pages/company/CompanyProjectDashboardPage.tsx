...
export function CompanyProjectDashboardPage(): ReactElement {
  ...
  return (
    <section className="company-page company-dashboard" aria-labelledby="company-dashboard-title">
      ...
      {state !== 'failed' && <>
        <section className="company-metric-grid" aria-label="项目概览"><article><span>服务中 / 验收中</span><strong>{state === 'loading' ? '…' : activeCount}</strong><small>{acceptanceCount > 0 ? `${acceptanceCount} 个项目正在验收` : '按项目状态自动汇总'}</small></article><article><span>草稿项目</span><strong>{state === 'loading' ? '…' : draftCount}</strong><small>{draftCount > 0 ? '进入项目档案核对后再转为服务中' : '当前没有待完善草稿'}</small></article><article><span>平台数据覆盖</span><strong className="company-metric-text">尚未接入</strong><small>不展示虚构的播放量、互动量或转化量</small></article><article><span>项目健康</span><strong className="company-metric-text">待建立基线</strong><small>由 Agent 读取交付记录后生成</small></article></section>
