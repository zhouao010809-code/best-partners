...
const cursorSchema = z.strictObject({ version: z.literal(1), search: z.string(), projectId: z.uuid().nullable().optional(), updatedAt: z.string().min(1).max(100), id: z.string().min(1).max(200) });
...
export function listAssistantHistory(database: Database.Database, input: AssistantHistoryQuery = {}): AssistantHistoryPage {
  ...
  if (query.cursor) {
    ...
    if ((cursor.projectId ?? undefined) !== query.projectId) throw new PublicApiError('ASSISTANT_HISTORY_CURSOR', '项目范围已变化，请重新读取。', 400);
  }
  ...
  return {
    ...
    ...(hasMore && last ? { nextCursor: Buffer.from(JSON.stringify({ version: 1, search, projectId: query.projectId ?? null, updatedAt: last.updated_at, id: last.id })).toString('base64url') } : {})
  };
}
