...
import { assistantDraftDeleteResponseSchema, assistantDraftListResponseSchema, assistantDraftQuerySchema, assistantDraftResponseSchema, assistantDraftSaveSchema } from '../../../shared/api/assistant-drafts.js';
...
export function registerAssistantDraftRoutes(app: FastifyInstance, options: { assistantDrafts?: AssistantDraftService | undefined }) {
  ...
  app.get('/api/v1/assistant/drafts', async (request, reply) => { reply.header('cache-control', 'no-store'); return parseApiOutput(assistantDraftListResponseSchema, { version: API_VERSION, data: required().list(parseApiInput(assistantDraftQuerySchema, request.query)) }); });
