import { z } from 'zod';
import { creativeProfileFieldsSchema, type CreativeProfileContext, type CreativeProfileSave, type ProjectCreativeProfile } from './creative-profile.js';
...
export const creationReferenceSelectionSchema = z.strictObject({
  mode: z.enum(['auto', 'selected']),
  paths: z.array(projectRelativePathSchema.refine(path => path !== 'AI工作区' && !path.startsWith('AI工作区/'), '不能引用 AI工作区 中的生成内容')).max(20)
}).superRefine((value, context) => {
  if (value.mode === 'auto' && value.paths.length !== 0) context.addIssue({ code: 'custom', path: ['paths'], message: '自动参考不能指定文件' });
  if (value.mode === 'selected' && value.paths.length === 0) context.addIssue({ code: 'custom', path: ['paths'], message: '请至少选择一份参考文件' });
  if (new Set(value.paths).size !== value.paths.length) context.addIssue({ code: 'custom', path: ['paths'], message: '参考文件不能重复' });
});
export type CreationReferenceSelection = z.infer<typeof creationReferenceSelectionSchema>;
...
const editableFields = {
  ...
  audience: z.string().max(2000), angle: z.string().max(2000), rationale: z.string().max(4000), sources: sourcesSchema, referenceSelection: creationReferenceSelectionSchema.optional()
};
...
export const creationVersionSchema = z.strictObject({
  ...
  body: bodySchema, sources: sourcesSchema, referenceSelection: creationReferenceSelectionSchema.optional(), createdAt: z.string()
});
...
export const creationSuggestionSchema = z.strictObject({
  id: z.uuid(), task: z.enum(['topics', 'script', 'revise', 'discuss', 'profile']), reply: z.string().max(60000),
...
export const creationGenerateRequestSchema = z.strictObject({
  task: z.enum(['topics', 'script', 'revise', 'discuss', 'profile']), instruction: z.string().trim().min(1).max(4000),
  itemId: z.uuid().optional(), expectedRevision: revisionSchema.optional(), model: z.string().min(1).max(200).optional(),
  referenceSelection: creationReferenceSelectionSchema.optional(),
  ...
  if (value.task === 'profile' && value.itemId !== undefined) context.addIssue({ code: 'custom', path: ['itemId'], message: '创作档案建议不能关联单条稿件' });
  if (value.itemId !== undefined && value.referenceSelection !== undefined) context.addIssue({ code: 'custom', path: ['referenceSelection'], message: '稿件生成使用已保存的参考范围' });
...
export interface ProjectCreationService {
  getProfile(projectId: string): Promise<ProjectCreativeProfile>;
