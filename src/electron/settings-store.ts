...
export async function resolveInitialVaultSettings(input: {
  ...
  readonly showInvalidSelection: () => Promise<void>;
  ...
  for (;;) {
    ...
    try { return await input.validate(selected); } catch { await input.showInvalidSelection(); }
  }
}
