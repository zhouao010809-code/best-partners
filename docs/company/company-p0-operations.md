...
Skill 正文中的命令只是不可信的参考内容，MCP 不执行它们；新增或修改 Skill 仍通过工作区文件和后续受控流程完成。网页上的“Agent 连接”只是连接说明，不是内置聊天窗口。项目详情里的“项目助理”只生成带当前项目范围的受控只读提示词，复制提示词不等于 Agent 已执行；任何项目写入仍走提案 → 人工确认流程。
