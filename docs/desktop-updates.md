...
从 v0.1.6 起，更新检查与手动安装包下载使用 Electron 的 Chromium 网络栈，沿用 macOS 系统代理；不读取或硬编码代理地址。下载逐跳验证官方域名，保留大小上限与 SHA-256 校验。

收到数据前显示“正在连接下载源…”，收到数据后显示已下载大小、总量和百分比。连接、首字节或传输连续 30 秒无进展时结束并提供重试，整次下载最多 15 分钟。取消或失败会停止网络请求并清理临时安装包。
