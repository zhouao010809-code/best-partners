...
5. 点击“查看并导出定稿”，或在版本记录中点击“导出此版本”，核对预览后“确认导出”，才会新建项目 `AI工作区/内容草稿` 下的 Markdown 文件。同标题的不同稿件、不同版本互不覆盖。定稿、导出和对外发布是不同动作，应用不会替你发布。
...
已有项目问答仍可用“讨论项目”打开；它的旧对话不会自动变成脚本。创作模型沿用 DeepSeek 配置，相关资料在你发起生成时发送给服务商。测试采用隔离资料与模拟模型；真实模型输出质量仍需实际使用检验。


### 项目创作档案与参考资料（v0.1.5）

- 项目顶部的“项目创作档案”默认收起。展开后填写受众、内容目标、表达风格、已确认事实和避免表达，点击“保存项目档案”后用于后续创作。可从资料整理候选，先预览、采用到表单，再保存；空档案不妨碍写稿。
- 可指定本项目最多 3 份已定稿版本作为表达范例。固定的是当时版本，后来改稿或重新定稿不会自动替换它。范例用于表达参考，不作为新事实来源。
- 在策划或稿件的“本条参考资料”中，可自动查找，或选择 1–20 份可读的项目原始文件并“应用资料选择”。所选模式限制本轮项目搜索与读取，并关闭全局知识检索；仍沿用保存的创作档案、当前稿件与范例。选择范围不代表每份资料都已完整阅读，具体以返回来源为准。
- 所选资料失效时，生成会先停止，需重新选择；不会悄悄改查全部文件。每条稿件的范围随草稿、版本、恢复和副本保留，策划生成的选题继承提交时的范围。修改范围或档案后，旧修改建议不能直接覆盖新稿。
- 编辑时项目状态和创作档案收起；“讨论项目”用于项目整体讨论，“打磨这篇”处理当前内容。保存版本后不强制展开版本记录，已有定稿可直接预览、确认导出。
