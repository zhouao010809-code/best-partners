...
- [x] **Step 2: 写独立 TypeScript 配置和 MCP Vitest 配置**
...
- [x] **Step 3: 写 fixture builder 和第一批失败断言**
...
- [x] **Step 1: 实现 `VaultReader` 的根校验和路径解析**
...
- [x] **Step 4: 实现 `tools.ts` 的 Zod schema 和四个 handler**
...
- [x] **Step 2: 做针对性验证**
  ...
  已执行的全局 `npm run test:unit` 结果为 869/871，通过项未受 MCP 影响；剩余 2 项分别是并行 company 改动导致的 bootstrap schema 断言和已有 MaterialDeck 来源路径断言，未修改它们。
