...
个人 macOS 大脑管理 App。当前交付包含本地读取、问问 AI 与文件导入、资料归档、DeepSeek 提炼、候选编辑、人工确认后的可恢复知识入库与来源找回，以及四来源统一回收站和提炼队列移出/重新加入；不提供 SaaS 服务。
...
- 明确说“归档这个文件”后，问问通过现有归档事务保留原件和带页码的 Markdown，并返回真实位置。再次处理同内容复用已有归档；同名不同内容默认创建带版本标记的新资料。明确说“提炼这个文件”会先归档来源，再生成所选范围的知识候选；正式写入知识库仍在候选页预览并确认。仅处理部分页时原资料保留“部分入库”，仍可继续提炼剩余内容，不把整份 PDF 标为完成。
...
原始资料留在所选大脑文件夹中。索引与设置保存在 App 的 `userData` 目录（正常为 `~/Library/Application Support/小兆大脑`）。设置位于 `config`，每个大脑按路径与目录身份使用独立的 `vaults/<hash>` 数据目录；切换文件夹不会混用旧库索引，也不删除旧目录。其 `personal-intake-v1` 保存不可自动删除的原文与操作前镜像，与数据库故障专用的 `recovery` 目录分开；这些前镜像不是可随便清空的索引缓存。归档要求此恢复目录和大脑位于同一卷，否则仅保留读取能力。退出 App 会停止本地服务。
...
此前正式大脑只读验收：299 份 Markdown 中 278 份可建立记录，21 份仍需分类确认（其中包含资料包附属章节），仍可在 App 内查看原文。兼容读取恢复了原先被排除的 70 份资料，未改动原文或附件。用户已在 App 中归档真实插件资料“重新思考 GPT-6 Astra 的技能和提示”，归档后状态为已归档、未提炼。所有自动写入、进程中断与冲突测试均使用独立临时测试库，不以正式大脑试写验证新功能。
