...
test(`${mode}: independent read-only desktop, isolated renderer and automatic refresh`, async ({}, testInfo) => {
  ...
  try {
    ...
    }))).toEqual({ process: 'undefined', require: 'undefined', bridge: ['chooseVaultDirectory', 'getAppVersion', 'getClipperStatus', 'getVaultInfo', 'installClipperHost', 'openAssistantLogin', 'openClipperInstall', 'openVaultDirectory', 'revealDocument', 'revealSkill'] });
