...
const launchOptions = mode === 'packaged'
  ? { executablePath: resolve('dist/desktop/小兆大脑-darwin-arm64/小兆大脑.app/Contents/MacOS/小兆大脑'), args: [] }
...
test(`${mode}: independent read-only desktop, isolated renderer and automatic refresh`, async ({}, testInfo) => {
  ...
  try {
    ...
    const material = await readFile(resolve('tests/fixtures/library-valid.md'));
    ...
    await expect(window).toHaveTitle('小兆大脑');
    ...
    }))).toEqual({ process: 'undefined', require: 'undefined', bridge: ['chooseVaultDirectory', 'getAppVersion'] });
