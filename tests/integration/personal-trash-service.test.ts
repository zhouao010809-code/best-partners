...
it('migrates existing recycle records without losing their recovery data or hidden queue settings',()=>{
  ...
  expect(versions.slice(0, -5)).toEqual([1,2,8,9,10,11,12,14,15,16].map((version)=>({version})));
  expect(versions.slice(-5)).toEqual([{version:17},{version:18},{version:19},{version:20},{version:21}]);
});
