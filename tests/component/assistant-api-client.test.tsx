...
it('sends assistant messages once with the existing CSRF transport and request id', async () => {
  const fetcher = vi.fn().mockResolvedValueOnce(ok({ csrfToken: 'a'.repeat(43), runtimeMode: 'personal' })).mockResolvedValueOnce(ok(conversation));
...
it('posts explicit stop and login requests to their own protected endpoints', async () => {
  const fetcher = vi.fn().mockResolvedValueOnce(ok({ csrfToken: 'a'.repeat(43), runtimeMode: 'personal' })).mockResolvedValueOnce(ok(conversation)).mockResolvedValueOnce(ok({ message: '已连接' }));
  const service = createBrowserReadConsoleApi(fetcher).assistant!;
...
it('uploads a binary File with stable batch and upload ids through CSRF without JSON encoding', async () => {
  ...
  const fetcher = vi.fn().mockResolvedValueOnce(ok({ csrfToken: 'a'.repeat(43), runtimeMode: 'personal' })).mockResolvedValueOnce(ok({ attachment }));
...
it('uses revision-bearing draft writes and exposes conflict codes without discarding the current draft', async () => {
  const fetcher = vi.fn().mockResolvedValueOnce(ok({ csrfToken: 'a'.repeat(43), runtimeMode: 'personal' })).mockResolvedValueOnce(new Response(JSON.stringify({ error: { code: 'ASSISTANT_DRAFT_CONFLICT', message: '其他窗口已更新', operationId: '73c8cd22-39f0-4574-a1b5-d2b034949543' } }), { status: 409 }));
...
it('deletes a specific draft revision without declaring an empty JSON body', async () => {
  const fetcher = vi.fn().mockResolvedValueOnce(ok({ csrfToken: 'a'.repeat(43), runtimeMode: 'personal' })).mockResolvedValueOnce(ok({ deleted: true }));
